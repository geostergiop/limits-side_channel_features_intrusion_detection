#!/usr/bin/env python3
"""Generate the complete, audited Phase 7 NDSS Markdown report."""
...
OUTPUT = RESULTS / "ndss_balanced_llm_local_paper_tables_2026-07-03.md"

SOURCE_PATHS = {
    "local_balanced": RESULTS / "ndss_local_results_balanced.json",
    "local_deployment": RESULTS / "ndss_local_results_deployment.json",
    "llm_balanced": RESULTS / "ndss_llm_results_balanced_paper_5k.json",
    "llm_deployment": RESULTS / "ndss_llm_results_deployment_paper_6k.json",
}
...
def build_report(data: dict[str, list[dict]], audit_notes: list[str]) -> str:
    ...
    lines = [
        "# NDSS Phase 7 Complete Results: Packet and Session Detection",
        "",
        ...
        *best_and_worst_text(local_by_mode, llm_by_mode),
        ...
        "",
        "Phase 7 crosses two evaluation modes, three feature sets, packet and session representations, and both "
    ...
    source_rows.extend([
        ...
        ["results/ndss_local_vs_limits2_comparison_2026-07-03.md", "legacy reconciliation", "27,368", "2026-07-03", "see source"],
    ])
    ...
    lines.extend([
        ...
        "",
        "## Comparison with Pre-NDSS Packet Experiments",
        "",
        "The older `limits2.tex` experiments are not paired reruns. They used earlier packet-centric cohorts and "
        "protocols, while Phase 7 uses frozen capture-grouped session/window/packet-ablation manifests and broader "
    ...
    legacy_rows = [
        ["Pre-NDSS", "CART E1 session", "packet-era grouped baseline", "98.98%", "98.27%"],
        ["Pre-NDSS", "KNN E1 random", "random packet split", "99.13%", "98.56%"],
        ["Pre-NDSS", "RF E1 capture", "capture-held packet baseline", "97.64%", "96.25%"],
        ["Phase 7 balanced", "best local session/window", f"{best_bal_local['algorithm']} / {best_bal_local['feature_set']} / {unit_label(best_bal_local)}", pct(best_bal_local["accuracy"], 2), pct(best_bal_local["f1_1"], 2)],
        ["Phase 7 balanced", "best GPT-5.5 session/window", f"{best_bal_llm['llm_context_mode']} / {best_bal_llm['feature_set']} / {unit_label(best_bal_llm)}", pct(best_bal_llm["accuracy"], 2), pct(best_bal_llm["f1_1"], 2)],
        ["Phase 7 deployment", "best local overall", f"{best_dep_local['algorithm']} / {best_dep_local['feature_set']} / {unit_label(best_dep_local)}", pct(best_dep_local["accuracy"], 2), pct(best_dep_local["f1_1"], 2)],
        ["Phase 7 deployment", "best GPT-5.5 overall", f"{best_dep_llm['llm_context_mode']} / {best_dep_llm['feature_set']} / {unit_label(best_dep_llm)}", pct(best_dep_llm["accuracy"], 2), pct(best_dep_llm["f1_1"], 2)],
    ]
    lines.extend(md_table(["Generation", "Result", "Configuration", "Accuracy", "F1(mal)"], legacy_rows))
    lines.extend([
        "The lower Phase 7 scores are credible and informative: they expose cross-capture generalization, temporal "
    ...
    lines.extend([
        "- Final and partial LLM artifacts are byte-identical for both balanced and deployment runs.",
        ...
        "headline calculations are reproducible. The evidence supports comparative Phase 7 claims about local versus "
...
def main() -> None:
    ...
    for stem in ["balanced_paper_5k", "deployment_paper_6k"]:
        final_path = RESULTS / f"ndss_llm_results_{stem}.json"
        partial_path = RESULTS / f"ndss_llm_results_{stem}.partial.json"
        if sha256(final_path) != sha256(partial_path):
            raise AssertionError(f"Final and partial artifacts differ for {stem}")
